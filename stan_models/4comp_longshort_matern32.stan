...
transformed parameters {
    ...
    for (i in 1:(T-1)) {
      K2b[i, i] = K2bdiag;
      for (j in (i + 1):T) { 
       tmp = pow(3,0.5) * sqrt(dot_self((x[i] - x[j]) ./ rho_gp2b));
        K2b[i, j] = sq_alpha2b * (1 + tmp) * exp(-1 * tmp) ;
        K2b[j, i] = K2b[i, j];
      }
    }
    K2b[T, T] = K2bdiag;
...
model {
    ...
    alpha_gp2 ~ normal(20,2); // scale
    
    rho_gp1b ~ normal(1,0.01); // lengthscale
    rho_gp2b ~ normal(1,0.01); // lengthscale
    alpha_gp1b ~ normal(3,1); // scale
    alpha_gp2b ~ normal(3,1); // scale
